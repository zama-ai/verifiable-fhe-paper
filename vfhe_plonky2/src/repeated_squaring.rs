...
fn main() -> Result<()> {
    ...
    let current_sqr_in = builder.add_virtual_target();
    let current_sqr_out =
        builder.mul(current_sqr_in, current_sqr_in);
    builder.register_public_input(current_sqr_out);
    let counter = builder.add_virtual_public_input();
    ...
    let inner_cyclic_initial_target = inner_cyclic_pis[0];
    let inner_cyclic_latest_sqr = inner_cyclic_pis[1];
    ...
    let actual_sqr_in =
        builder.select(condition, inner_cyclic_latest_sqr, initial_target);
    builder.connect(current_sqr_in, actual_sqr_in);
    ...
    let initial = [F::from_canonical_usize(3)];
    ...
    let initial = proof.public_inputs[0];
    let rep_sqr = proof.public_inputs[1];
    let counter = proof.public_inputs[2];

    println!(
        "x^(2^n) where x = {} and n = {} is {}",
        initial,
        counter,
        rep_sqr
    );
